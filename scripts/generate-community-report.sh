#!/usr/bin/env bash
#
# generate-community-report.sh - Generate community/developer reports for F1R3FLY.io
#
# Produces a developer-community-oriented report covering product updates,
# technical highlights, contributor recognition, and contribution opportunities.
#
# Usage:
#   generate-community-report.sh [OPTIONS]
#
...
#
# Examples:
#   ./generate-community-report.sh                        # Monthly for previous month
#   ./generate-community-report.sh --month 2026-02        # Monthly for Feb 2026
#   ./generate-community-report.sh --biweekly             # Last 14 days
#   ./generate-community-report.sh --biweekly 2026-02-24  # 14 days from date
#   ./generate-community-report.sh --dry-run              # Preview
#
...
source "${LIB_DIR}/community-report-formatting.sh"
...
usage() {
    cat <<EOF
generate-community-report.sh - Generate F1R3FLY.io community/developer reports

Usage: generate-community-report.sh [OPTIONS]
...
setup_period() {
    case "$REPORT_TYPE" in
        monthly)
            ...
            DEFAULT_OUTPUT="${REPO_ROOT}/docs/reports/${TARGET_PERIOD}-community-report.md"
            ;;
        biweekly)
            ...
            DEFAULT_OUTPUT="${REPO_ROOT}/docs/reports/${TARGET_PERIOD}-biweekly-community-report.md"
            ;;
    esac
...
main() {
    ...
    echo -e "${BOLD}+--------------------------------------------------+${RESET}"
    echo -e "${BOLD}|   F1R3FLY.io Community Report Generator          |${RESET}"
    echo -e "${BOLD}+--------------------------------------------------+${RESET}"
    echo ""
    ...
    log INFO "Generating community report..."

    local generated_at
    generated_at=$(date -u '+%Y-%m-%dT%H:%M:%SZ')

    local report
    report=$(generate_community_report "$all_data" "$PERIOD_NAME" "$TARGET_PERIOD" "$generated_at" "$CADENCE" "$START_DATE" "$END_DATE")
    ...
    if [[ "$DRY_RUN" == true ]]; then
        ...
        echo ""
        validate_community_report "$OUTPUT_FILE"
        echo ""
        log INFO "Next steps:"
