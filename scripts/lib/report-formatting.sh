...
generate_report() {
    ...
    local report_type="${5:-monthly}"
    local prev_data="${6:-}"

    # Set doc_type and title based on report type
    local doc_type report_title
    case "$report_type" in
        monthly) doc_type="monthly-report"; report_title="Monthly Status Report" ;;
        daily)   doc_type="daily-report"; report_title="Daily Status Report" ;;
        mtd)     doc_type="mtd-report"; report_title="Month-to-Date Status Report" ;;
        annual)  doc_type="annual-report"; report_title="Annual Status Report" ;;
        *)       doc_type="status-report"; report_title="Status Report" ;;
    esac
